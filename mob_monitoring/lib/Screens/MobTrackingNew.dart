...
class MobTrackingScreenNew extends StatefulWidget {
  final List<Mob> mlist;
  const MobTrackingScreenNew({required this.mlist});
  @override
  State<MobTrackingScreenNew> createState() => _MobTrackingScreenNewState();
}

class _MobTrackingScreenNewState extends State<MobTrackingScreenNew> {
  List<List<Mob>> allParentsMobs = [];
  List<List<Drone>> allParentsAssignDrone = [];
  List<List<Station>> allParentStation = [];
  List<List<int>> mobids = [];
  List<List<dynamic>> MSD = [[], [], []];
  TextEditingController strengthCT = TextEditingController();
  final List<VideoPlayerController?> _videoControllers =
      List<VideoPlayerController?>.generate(100, (index) => null);
  ...
  void initState() {
    ...
    for (int i = 0; i < all_mlist.length; i++) {
      ...
      mobids.add([]);
      allParentsMobs.add([]);
      allParentStation.add([]);
      allParentsAssignDrone.add([]);
    }
    loaddetails();
  }
  ...
  void loaddetails() async {
    for (int i = 0; i < all_mlist.length; i++) {
      ...
      MobTracking mt = await APIHandler().saveSingleMovementAndGet(
          all_mlist[i].id,
          DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now()),
          all_mlist[i].location.latitude,
          all_mlist[i].location.longitude,
          all_mlist[i].location.lname,
          i + 1,
          1,
          0,
          0);
  ...
  Future<void> terminateMob(int id) async {
    await APIHandler().terminateMob(id, "Completed",
        DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now()));
  }
              ...
              child: Expanded(
                child: GoogleMap(
                  ...
                  onTap: (LatLng latLng) async {
                    if (mbaddlist.contains(mobIndex)) {
                      showDialog(
                          context: context,
                          builder: (context) {
                            return AlertDialog(
                              ...
                              backgroundColor: Colors.white,
                              title: Text(
                                '${mlist[mobIndex]} \nMob ${mobIndex + 1} is terminated \nPLease Select another mob',
...
//AIk
                      ...
                      MobTracking mtt = await APIHandler()
                          .saveSingleMovementAndGet(
                              ...
                              id,
                              1,
                              markeeed == false ? id - 1 : spid,
                              markeeed == false
                                  ? trackinglist[mobIndex].last.splitId
                                  : spid);
                      trackinglist[mobIndex].add(mtt);

                      print(
                          "@@@@@@@@@@@@@@@@@@@@@@@@  Move Id is ${mtt.moveId} now upload video   @@@@@@@@@@@@@@@@@@@@@@@@");
                      movessid = mtt.moveId;
                      ...
                      if (markeeed == true) {
                        ...
                        if (ind + 1 < trackinglist[mobIndex].length - 1) {
                          int c = trackinglist[mobIndex]
                              .where((element) => element.parentId == spid)
                              .length;
                          if (c > 1) {
                            print("childddddddddddddddddddddddddddddddd $c");
                            var res = await APIHandler().addParentMob(
                                all_mlist[mobIndex].type,
                                DateFormat('yyyy-MM-dd HH:mm:ss')
                                    .format(DateTime.now()),
                                newMarker.position.latitude,
                                newMarker.position.longitude,
                                ...
                                "Active",
                                all_mlist[mobIndex].id,
                                userInfo[0]["user"]["id"] == null
                                    ? 9
                                    : userInfo[0]["user"]["id"]);

                            var msd = jsonDecode(res.body);

                            allParentsMobs[mobIndex].add(Mob.fromMap(msd[0]));
                            allParentStation[mobIndex]
                                .add(Station.fromMaps(msd[1]));
                            allParentsAssignDrone[mobIndex]
                                .add(Drone.fromMap(msd[2]));

                            MSD[0].add(Mob.fromMap(msd[0]));
                            MSD[1].add(Station.fromMaps(msd[1]));
                            MSD[2].add(Drone.fromMap(msd[2]));

                            mobids[mobIndex]
                                .add(allParentsMobs[mobIndex].last.id);
                          }
                        }
                      }
                      // int ind=    trackinglist[mobIndex].indexWhere((element) => element.markerId==id);
                      // trackinglist[mobIndex][ind]=mtt;
                      markeeed = false;
                      IsSplitColor = false;
                      completeMob = false;
                      mklist.add(id);
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            movementslist[mobIndex]
                                                    ...
                                                    .isNotEmpty
                                                ? Text(
                                                    "Strength : ${movementslist[mobIndex].firstWhere((element) => element.markid == id).head_cout}",
                                                    style:
                  ...
                  markers: Set<Marker>.of(markers.map((marker) {
                    return Marker(
                      ...
                      onTap: () async {
                        ...
                        mklist.add(spid);
                        print(mklist.length);
                        print("Mmm ${trackinglist[mobIndex].length}");
                                                                        ...
                                                                        builder:
                                                                          ...
                                                                          return AlertDialog(
                                                                            ...
                                                                            title:
                                                                              ...
                                                                              children: [
                                                                                Text(
                                                                                  mlist.length == 1 ? "Mob Is Terminated" : "All Mobs has been Terminated",
                                                                        ...
                                                                        return AlertDialog(
                                                                          ...
                                                                          title:
                                                                            ...
                                                                            child:
                                                                              ...
                                                                              children: [
                                                                                Text(
                                                                                  "${mlist[mobIndex]} \nMob ${mobIndex + 1} is terminated \nBecause strength is $a that less than 50",
            ...
            Row(
              ...
              children: [
                ...
                //         completeMob = false;
                //         markerStop = false;
                //         if (mmlist.length == mbaddlist.length) {
                //           IsTrack == true;
                //           completeMob = true;
                //           markerStop = true;
                //         }
                ...
                InkWell(
                  ...
                  onTap: () async {
                    if (mbaddlist.contains(mobIndex)) {
                          ...
                          builder: (context) {
                            return AlertDialog(icon:   Icon(
                              ...
                              title: Row(
                                children: [
                                  Text(
                                    '${mlist[mobIndex]} \nMob ${mobIndex + 1} is already terminated ',
                                    style: TextStyle(fontSize: 14),
                                  ),
                      ...
                      for (int i = 0; i < mobids[mobIndex].length; i++) {
                        await APIHandler().terminateMob(
                            mobids[mobIndex][i],
                            "Completed",
                            DateFormat('yyyy-MM-dd HH:mm:ss')
                      ...
                      mbaddlist.add(mobIndex);
                      if (mlist.length == mbaddlist.length) {
